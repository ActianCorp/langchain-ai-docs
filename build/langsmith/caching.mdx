...
Graphs deployed on [Agent Server](/langsmith/agent-server) often call external services — config stores, auth providers, feature-flag systems — that return data you don't need to re-fetch on every invocation. The built-in cache lets you store those results server-side so subsequent graph runs read from memory instead of making redundant network calls.
