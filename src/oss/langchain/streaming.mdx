...
<Note>
If completed messages are tracked in an agent's [state](/oss/langchain/agents#memory), you can use `stream_mode=["messages", "updates"]` as demonstrated in the [Streaming tool calls](#streaming-tool-calls) section to access completed messages during streaming.
</Note>
