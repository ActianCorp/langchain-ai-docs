---
title: Azure Cosmos DB for MongoDB vCore (deprecated)
description: "Integrate with the Azure Cosmos DB for MongoDB vcore vector store using LangChain JavaScript."
---

<Warning>
**This integration has been renamed.** Azure Cosmos DB for MongoDB vCore has been rebranded to Azure DocumentDB by Microsoft.

Please use the new [Azure DocumentDB](/oss/integrations/vectorstores/azure_documentdb) integration instead. The old class names (`AzureCosmosDBMongoDBVectorStore`, etc.) are still available as deprecated aliases for backwards compatibility, but you should migrate to the new `AzureDocumentDBVectorStore` class.
</Warning>
