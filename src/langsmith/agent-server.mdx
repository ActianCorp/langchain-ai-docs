...
[PostgreSQL](https://www.postgresql.org/) is supported as a database for Agent Server and [Redis](https://redis.io/) as the task queue. Checkpoints can optionally be stored in MongoDB instead of PostgreSQL. See [Configure checkpointer backend](/langsmith/configure-checkpointer) for details.
