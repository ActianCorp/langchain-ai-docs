...
A **snapshot** is a filesystem bundle backed by a Docker image. It is the required input to every sandbox: `createSandbox` / `create_sandbox` always takes a snapshot ID. Build a snapshot once, then boot as many sandboxes from it as you need.
...
Start a sandbox from an existing snapshot, install packages or prepare data, then capture the result as a new snapshot.

<CodeGroup>

```python Python
sb = client.create_sandbox(snapshot_id=base_snapshot_id, name="setup-box")
sb.run("pip install numpy pandas scikit-learn", timeout=120)
...
await running.run("pip install numpy pandas scikit-learn", { timeout: 120 });
