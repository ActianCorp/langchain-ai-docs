...
# This includes store operations, crons, and any other resource without a specific handler.
@auth.on
...
| **Store** | `@auth.on.store.put` | Store item creation/update | `dict` |
| | `@auth.on.store.get` | Store item retrieval | `dict` |
| | `@auth.on.store.delete` | Store item deletion | `dict` |
| | `@auth.on.store.search` | Store search | `dict` |
| | `@auth.on.store.list_namespaces` | Listing namespaces | `dict` |
...
Store operations use namespace-based access control rather than metadata filters. Unlike threads and assistants, store handlers receive a `dict` with fields like `namespace`, `key`, and `value` instead of typed objects. The global `@auth.on` handler catches store operations along with all other resources. For fine-grained control, use `@auth.on.store()` to handle all store actions or action-specific handlers like `@auth.on.store.put`.
</Note>
