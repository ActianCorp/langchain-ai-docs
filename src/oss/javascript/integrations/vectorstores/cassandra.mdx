...
vector store is first created, and specified in the constructor parameter `vectorType`, for example:
...
This facilitates bind values on the left of the operator, which is useful for some functions; for example
