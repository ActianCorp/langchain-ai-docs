...
The `feedback_stats` field's `value_counts` breakdown only includes feedback with string values. Feedback with non-string values (numeric, boolean, complex types) is excluded from `value_counts` but still contributes to aggregate statistics such as `avg`, `n`, and `stdev`. If you need the full distribution of non-string feedback values, export the raw feedback data separately.
</Note>
