...
JSON-encoded configuration for the checkpointer backend. Controls which storage system Agent Server uses for [checkpoint](/oss/langgraph/persistence) data.
...
When set, takes precedence over the `checkpointer` key in `langgraph.json`. See [Configure checkpointer backend](/langsmith/configure-checkpointer) for available backends and options.
...
Fallback MongoDB connection URI for the MongoDB checkpointer backend. Used only when neither the `uri` field in [`LANGGRAPH_CHECKPOINTER`](#langgraph_checkpointer) nor [`LS_MONGODB_URI`](#ls_mongodb_uri) is set.
...
MongoDB connection URI for the MongoDB checkpointer backend. Used when the `uri` field is not set in [`LANGGRAPH_CHECKPOINTER`](#langgraph_checkpointer). Takes precedence over [`MONGODB_URI`](#mongodb_uri).
