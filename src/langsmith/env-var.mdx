...
## `DEFAULT_A2A_ASSISTANT_ID`

Set `DEFAULT_A2A_ASSISTANT_ID` to the UUID of the assistant to use for the [A2A (Agent-to-Agent)](https://google.github.io/A2A/) protocol endpoint (`/.well-known/agent-card.json`) when no `assistant_id` query parameter is provided.

If not set, the `assistant_id` query parameter is required on every A2A request.
...
Set `LANGCHAIN_REVISION_ID` to the git commit SHA of your deployed code. This value is used by the LangSmith SDK for [runtime environment inference](https://docs.smith.langchain.com/observability/how_to_guides/tracing/annotate_code#runtime-environment) and will appear in LangSmith trace metadata.

If not set, the server will fall back to the internally-set `LANGSMITH_LANGGRAPH_GIT_REF_SHA` value when available.

## `LANGGRAPH_AES_KEY`

<Info>
**Only for Hybrid and Self-Hosted**
Field-level AES encryption is only available for [Hybrid](/langsmith/hybrid) and [Self-Hosted](/langsmith/self-hosted) deployments.
</Info>

Set `LANGGRAPH_AES_KEY` to enable AES encryption of selected metadata fields at rest. The key must be exactly 16, 24, or 32 bytes (UTF-8 encoded) for AES-128, AES-192, or AES-256 respectively.

Use this together with [`LANGGRAPH_AES_JSON_KEYS`](#langgraph_aes_json_keys) to specify which metadata keys to encrypt.

## `LANGGRAPH_AES_JSON_KEYS`

Comma-separated list of metadata JSON keys to encrypt at rest using the key provided in [`LANGGRAPH_AES_KEY`](#langgraph_aes_key). Requires `LANGGRAPH_AES_KEY` to be set.

For example: `LANGGRAPH_AES_JSON_KEYS=api_key,secret_token`

The following system-populated keys cannot be encrypted: `langgraph_version`, `langgraph_api_version`, `langgraph_plan`, `langgraph_host`, `langgraph_api_url`, `langgraph_request_id`, `langgraph_auth_user_id`, `langgraph_auth_permissions`.

## `LANGGRAPH_API_URL`

Set `LANGGRAPH_API_URL` to the publicly accessible URL of the Agent Server. This value is included in server metadata and can be used by clients for service discovery.

## `LANGGRAPH_DEFAULT_RECURSION_LIMIT`

Set the default [recursion limit](https://langchain-ai.github.io/langgraph/concepts/low_level/#recursion-limit) for graph execution. The recursion limit controls the maximum number of supersteps a graph can execute before raising an error, preventing infinite loops.
...
Set `LANGSMITH_OTEL_ENABLED` to `true` to enable OpenTelemetry tracing for [LangSmith SDK](https://docs.smith.langchain.com/) calls made by the Agent Server. When enabled, LangSmith trace data is exported via OpenTelemetry in addition to the default LangSmith backend.

This is separate from [`LS_APM_OTEL_ENABLED`](#ls_apm_otel_enabled), which controls APM-level instrumentation of the server itself. `LANGSMITH_OTEL_ENABLED` specifically controls whether the LangSmith SDK emits spans via OTLP.

Requires an OTLP endpoint to be configured (e.g. `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`).
...
Set `MOUNT_PREFIX` to serve the Agent Server under a specific path prefix. This is useful for deployments where the server is behind a reverse proxy or load balancer that requires a specific path prefix. `LANGGRAPH_MOUNT_PREFIX` is also accepted and takes precedence over `MOUNT_PREFIX`.

For example, if the server is to be served under `https://example.com/langgraph`, set `MOUNT_PREFIX` to `/langgraph`.

The prefix must start with `/` and must not end with `/`. The path `/noauth` is reserved and cannot be used as a prefix.
...
## `LSD_CACHE_PACKAGES_DISTRIBUTIONS`

Set `LSD_CACHE_PACKAGES_DISTRIBUTIONS` to `true` to cache the result of Python's `importlib.metadata.packages_distributions()`. This can significantly reduce import time when using packages (such as `google-api-core`) that call `packages_distributions()` repeatedly at import time, especially when Datadog's `ddtrace` import hooks are active.

The installed-package set does not change during a process lifetime, so caching is safe. Enable this if you observe slow graph import times in production.

Defaults to `false`.

## `MONGODB_URI`

MongoDB connection URI, used as a fallback when [`LS_MONGODB_URI`](#ls_mongodb_uri) is not set. This variable is only relevant when using the MongoDB [checkpointer backend](/langsmith/configure-checkpointer) (`LS_DEFAULT_CHECKPOINTER_BACKEND=mongo` or `checkpointer.backend: "mongo"` in `langgraph.json`).

`LS_MONGODB_URI` takes precedence if both are set.
