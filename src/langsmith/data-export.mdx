...
|-------|-------------|
| `tags` | List of tags |
| `feedback_stats` | Feedback statistics (JSON). See the note below for aggregation limitations. |

<Note>
**`feedback_stats` aggregation limitation**

The `feedback_stats` field only aggregates feedback with simple string values. Feedback with non-string values, such as numeric, boolean, or complex or object types, is excluded from the `feedback_stats` aggregation. If you need to analyze non-string feedback, export the raw feedback data separately.
</Note>
