...
Deep Agents run on LangGraph, which provides [durable execution](/oss/javascript/langgraph/durable-execution) out of the box. The [persistence](/oss/javascript/langgraph/persistence) layer checkpoints state at each step, so a run interrupted by a failure, timeout, or [human-in-the-loop](/oss/javascript/langgraph/interrupts) pause resumes from its last recorded state without reprocessing previous steps. For long-running deep agents that spawn many subagents, this means a mid-run failure doesn't lose completed work.
