...
Set `LSD_CACHE_PACKAGES_DISTRIBUTIONS` to `true` to cache Python package-distribution metadata. This can reduce graph import times when Datadog APM (`ddtrace`) is enabled.
