...
**OAuth via Agent Auth.** [Agent Auth](/langsmith/agent-auth) provides a managed OAuth 2.0 flow. Configure an OAuth provider, and the agent can request tokens scoped to each user. On first use, the agent [interrupts](/oss/langgraph/interrupts) execution and presents an OAuth consent URL. After the user authenticates, the agent resumes with a valid token. Tokens are stored and refreshed automatically.
...
**Workspace secrets.** For API keys shared across all users (for example your organization's LLM provider keys, search API keys), store them as [workspace secrets](/langsmith/setup) in LangSmith. See [Managing secrets](#managing-secrets) for details.
    ...
    new StateBackend(),
    {
      "/memories/": new StoreBackend({
        namespace: (ctx) => {
          ...
          return [ctx.runtime.context.userId];
        },
      }),
    },
  ),
...
You can also read and write to the store from your application code using the [Store API](/langsmith/custom-store). See [accessing memories from external code](/oss/deepagents/memory#accessing-memories-from-external-code) for examples.

For the full namespace factory API, see [namespace factories](/oss/deepagents/backends#namespace-factories). For memory patterns like self-improving instructions and knowledge bases, see [long-term memory](/oss/deepagents/memory).
...
- **Seed the sandbox before the agent runs**: upload user files, [skill](/oss/deepagents/skills) scripts, configuration, or [persistent memories](/oss/deepagents/memory) so the agent has what it needs from the start
...
<Accordion title="Example: syncing skills and memories with custom middleware">
  [Skill](/oss/deepagents/skills) scripts that the agent needs to execute must be uploaded into the sandbox before the agent runs. You may also want to sync [memories](/oss/deepagents/memory) so the agent can read and update them inside the container. Use [custom middleware](/oss/langchain/middleware/custom) with `before_agent` and `after_agent` hooks to move files across the sandbox boundary:
...
The `${SECRET_KEY}` references resolve against secrets stored in your LangSmith [workspace settings](/langsmith/setup). Configure secrets there before creating a template that references them.

**Workspace secrets.** For API keys that don't need proxy-based injection (e.g., keys used by the agent server itself, not sandbox code), store them as [workspace secrets](/langsmith/setup) in LangSmith. These are available as environment variables at runtime for all agents in the workspace.
